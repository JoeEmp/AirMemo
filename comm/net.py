import requests
from config import ENV, DOMAINS
...
def url(url: str, env=ENV):
    def decorator(func):
        def wrapper(*args, **kwargs):
            try:
                prefix = DOMAINS[env].value+'/'
            except:
                prefix = DOMAINS['debug'].value+'/'
...
class air_api(base_api):
    def __init__(self, *args, **kwargs):
        token = kwargs.get('token', None)
        self.headers = {'token': token}
        super().__init__(*args, **kwargs)

    def get_api(self, url, method='post', *args, **kwargs):
        if not kwargs.get('headers', None) and self.headers:
            return super().get_api(url, method=method, headers=self.headers, *args, **kwargs)
        else:
            return super().get_api(url, method=method, *args, **kwargs)
